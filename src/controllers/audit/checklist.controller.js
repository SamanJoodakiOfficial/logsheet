...
const newChecklist = async (req, res) => {
    try {
      const { name, description, times } = req.body; // اضافه کردن times
      ...
      const newChecklist = new Checklist({
        name,
        description,
        times, // اضافه کردن times به ساختار داده‌های چک‌لیست
      });
  ...
  try {
    ...
    if (!course) {
      return flashAndRedirect(
        ...
        res,
        "error",
        req.__("audit_course_not_found"),
        "/audit-courses"
      );
    }

    const userAccess = course.access.find(
    ...
    for (const access of course.access) {
      if (access.user.toString() === userId) {
        for (const checklist of access.checklists) {
          if (checklist) {
            checklist.questions = await Question.find({
    ...
    res.render("audit/auditCourses/checklists", {
      title: "پرسشنامه‌ها",
      course,
